 ...
 */
class DefaultFlameSpec extends FlameTestBase {

  "A Flame" should "ensure that length > 0" in {
    intercept [IllegalArgumentException] {
      DefaultFlame(length=0.0, angle=1.0, origin=Coord.Origin, depthIgnited = 0.0, deltaTemperature=300.0)
    }
  }
  
  it should "ensure that depth ignited >= 0" in {
    intercept [IllegalArgumentException] {
      DefaultFlame(length=0.1, angle=1.0, origin=Coord.Origin, depthIgnited = -0.1, deltaTemperature=300.0)
    }
  }
  
  it should "ensure that length > depth ignited" in {
    intercept [IllegalArgumentException] {
      DefaultFlame(length=0.1, angle=1.0, origin=Coord.Origin, depthIgnited = 0.2, deltaTemperature=300.0)
    }
  }
  
  it should "return the correct plume" in {
    forAll (coords, angles) { (origin, angle) =>
      val flame = DefaultFlame(0.5, angle, origin, 0.1, 300.0)
  ...
  it should "create a Flame with a new origin" in {
    forAll (coords, coords) { (origin, newOrigin) => 
      val flame = DefaultFlame(0.5, 0.0, origin, 0.1, 300.0)
      val newFlame = DefaultFlame.toOrigin(flame, newOrigin)
  ...
  it should "return plume temperature equal to plume delta temperature plus ambient" in {
    ...
    forAll (ambientTemps, flameTemps, distances) { (ambientT, flameT, d) => 
      whenever (d > depthIgnited) {
        val flame = DefaultFlame(depthIgnited * 2, 0.0, Coord.Origin, depthIgnited, flameT)
  ...
  it should "return the flame temperature for distances <= depth ignited and lower temperatures otherwise" in {
    ...
    val flame = DefaultFlame(flameLength, 0.0, Coord.Origin, depthIgnited, t)
  ...
  it should "return None for distance to a temperature > flame + ambient temperature" in {
    val t = 300.0
    val flame = DefaultFlame(1.0, 0.0, Coord.Origin, 0.5, t)
  ...
  it should "return depth ignited for the distance to the flame temperature" in {
    ...
    forAll (distances, angles) { (depth, angle) =>
      whenever (depth > 0.1) {
        ...
        val flame = DefaultFlame(flameLength, angle, Coord.Origin, depth, flameTemp)
  ...
  it should "calculate distanceForTemperature values which agree with plumeTemperature values" in {
    ...
    val flame = DefaultFlame(flameLength, 0.0, Coord.Origin, depthIgnited, flameTemp)
  ...
  "DefaultFlame.flameAngle" should "return 0 for flame angle when flame length is 0" in {
    forAll (windSpeeds, slopes, distances) { (wind, slope, fireLineLength) =>
      DefaultFlame.flameAngle(0.0, wind, slope, fireLineLength) should be (0.0)
    } 
  }
